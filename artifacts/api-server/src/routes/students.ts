  ...
  async (req, res): Promise<void> => {
    ...
    const [
      ...
      db
        ...
        .where(eq(certificationsTable.userId, uid)),
      db
        .select()
        .from(internshipsTable)
        .where(eq(internshipsTable.userId, uid)),
      db
        .select()
        .from(interviewsTable)
        .where(
          and(
            eq(interviewsTable.studentId, uid),
    ...
    const eligibleCount = companies.filter((c) => {
      ...
      return (
        cgpa >= parseFloat(c.minCGPARequired) &&
        (profile?.backlogs ?? 0) <= c.maxBacklogs
      );
    }).length;
    ...
    const [lastAnalysis] = await db
      ...
      .where(eq(aiAnalysesTable.userId, uid))
      .orderBy(sql`${aiAnalysesTable.analysisDate} DESC`)
      .limit(1);
    ...
    res.json({
      profile: profileData,
      readinessScore: lastAnalysis
        ? parseFloat(lastAnalysis.readinessScore)
        : 0,
  ...
  async (req, res): Promise<void> => {
    ...
    const [[user], [profile], [department]] = await Promise.all([
      ...
      db
        .select()
        .from(departmentsTable)
        .innerJoin(usersTable, eq(usersTable.departmentId, departmentsTable.id))
        .where(eq(usersTable.id, uid)),
    ]);
  ...
  async (req, res): Promise<void> => {
    ...
    if (existing) {
      [profile] = await db
        ...
        .set({ rollNo, batch, phone, address, bio, linkedinUrl, photoUrl })
        .where(eq(studentProfilesTable.userId, uid))
        .returning();
    } else {
      [profile] = await db
    ...
    const [updatedUser] = await db
      ...
      .where(eq(usersTable.id, uid))
      .returning();
    const [department] = updatedUser.departmentId
      ? await db
          .select()
          .from(departmentsTable)
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const [[profile], semesters] = await Promise.all([
      ...
      db
        .select()
        .from(semesterRecordsTable)
        .where(eq(semesterRecordsTable.userId, uid))
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    if (existing) {
      await db
        .update(studentProfilesTable)
        .set({ cgpa: String(cgpa), backlogs })
        .where(eq(studentProfilesTable.userId, uid));
    } else {
      await db
        .insert(studentProfilesTable)
        .values({ userId: uid, cgpa: String(cgpa), backlogs });
    }

    if (Array.isArray(semesters) && semesters.length > 0) {
      await db
        .delete(semesterRecordsTable)
        .where(eq(semesterRecordsTable.userId, uid));
...
router.get(
  ...
  async (req, res): Promise<void> => {
    const skills = await db
      .select()
      .from(skillsTable)
      .where(eq(skillsTable.userId, req.user!.userId));
    res.json(skills);
  },
);
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!name || !level || !category) {
      res.status(400).json({ error: "name, level and category are required" });
      return;
    }
    const [skill] = await db
      .insert(skillsTable)
      .values({ userId: req.user!.userId, name, level, category })
      .returning();
...
router.delete(
  ...
  async (req, res): Promise<void> => {
    ...
    await db
      .delete(skillsTable)
      .where(
        and(eq(skillsTable.id, id), eq(skillsTable.userId, req.user!.userId)),
      );
    res.sendStatus(204);
  },
);
    ...
    const [project] = await db
      ...
      .where(
        and(
          ...
          eq(projectsTable.userId, req.user!.userId),
        ),
      )
      .returning();
    if (!project) {
...
router.get(
  ...
  async (req, res): Promise<void> => {
    const rows = await db
      .select()
      .from(internshipsTable)
      .where(eq(internshipsTable.userId, req.user!.userId));
    res.json(
      rows.map((i) => ({
        ...i,
        stipend: i.stipend ? parseFloat(i.stipend) : null,
      })),
    );
  },
);
  ...
  async (req, res): Promise<void> => {
    ...
    const [intern] = await db
      ...
      .values({
        ...
        startDate,
        endDate,
        stipend: stipend != null ? String(stipend) : null,
        location,
      })
      .returning();
    res
      ...
      .json({
        ...intern,
        stipend: intern.stipend ? parseFloat(intern.stipend) : null,
      });
  },
);
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const body = {
      ...req.body,
      stipend: req.body.stipend != null ? String(req.body.stipend) : undefined,
    };
    const [intern] = await db
      ...
      .where(
        and(
          eq(internshipsTable.id, id),
          eq(internshipsTable.userId, req.user!.userId),
        ),
      )
      .returning();
    if (!intern) {
  ...
  async (req, res): Promise<void> => {
    res.json(
      await db
        .select()
        .from(certificationsTable)
        .where(eq(certificationsTable.userId, req.user!.userId)),
    );
  },
);
...
router.post(
  ...
  requireRole("student"),
  async (req, res): Promise<void> => {
    const { name, issuer, issueDate, expiryDate, credentialUrl, credentialId } =
      req.body;
    if (!name || !issuer) {
      res.status(400).json({ error: "name and issuer are required" });
      return;
    }
    const [cert] = await db
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(certificationsTable.userId, req.user!.userId),
        ),
      );
    res.sendStatus(204);
  },
...
router.get(
  ...
  async (req, res): Promise<void> => {
    const [row] = await db
      ...
      .where(eq(codingProfilesTable.userId, req.user!.userId));
    res.json(row ?? {});
  },
);
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    const [existing] = await db
      .select()
      .from(codingProfilesTable)
      .where(eq(codingProfilesTable.userId, uid));
    let row;
    if (existing) {
      [row] = await db
...
async function runAnalysis(uid: number, res: any): Promise<void> {
  const [[profile], skills, projects, certs, internships, [coding]] =
    await Promise.all([
      db
        .select()
        .from(studentProfilesTable)
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const result = companies.map((c) => {
      ...
      const eligible =
        cgpa >= parseFloat(c.minCGPARequired) && backlogs <= c.maxBacklogs;

      if (cgpa >= parseFloat(c.minCGPARequired))
        reasons.push(`CGPA ${cgpa} meets minimum ${c.minCGPARequired}`);
      ...
      if (backlogs <= c.maxBacklogs) reasons.push("Backlog count within limit");
        ...
        missing.push(`${backlogs} backlogs exceed limit of ${c.maxBacklogs}`);

      const reqSkills = c.requiredSkills ?? [];
      const matchedSkills = reqSkills.filter((rs) =>
        skillNames.some((sn) => sn.includes(rs.toLowerCase())),
      );
      ...
      const matchScore = Math.round(
        (reasons.length / Math.max(reasons.length + missing.length, 1)) * 100,
      );
