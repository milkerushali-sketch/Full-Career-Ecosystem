...
```sh
bash .local/secondary_skills/migrate-to-multi-artifact/references/fullstack-copy-backend.sh
```
...
Call `markTaskComplete({...})` as the final callback in CodeExecution — the user will verify visual parity before the next task starts.
